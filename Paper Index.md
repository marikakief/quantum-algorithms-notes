...
- [[Entanglement Accelerates Quantum Simulation (Zhao-Zhou-Childs 2025) — Paper Notes|Zhao-Zhou-Childs (2025)]] — local entropy deficits on Trotter-error supports interpolate between worst-case and average-case error; product states can be worst-case; approximate $k$-uniformity and checkpoint measurements recover average-case behavior
- [[Toward the First Quantum Simulation with Quantum Speedup (Childs-Maslov-Nam-Ross-Su 2018) — Paper Notes|Childs-Maslov-Nam-Ross-Su (2018)]] — end-to-end T-gate resource estimate for FeMo cofactor using 12th-order Suzuki formulas; error-budget splitting between Trotter and rotation synthesis; 12th order beats 2nd by $\sim 10^3\times$ here; $\sim 10^{14}$ T gates
...
- [[Locality and Digital Quantum Simulation of Power-Law Interactions (Tran-Guo-Su-Childs-Gorshkov 2019) — Paper Notes|Tran-Guo-Su-Childs-Gorshkov (2019)]] — tight Trotter error bounds for $1/r^\alpha$ power-law Hamiltonians via near/far decomposition; three $\alpha$-regimes (short-range, intermediate, all-to-all); LR light-cone lower bounds confirming near-optimality
...
- [[Quantum Simulation of Real-Space Dynamics (Childs-Leng-Li-Liu-Zhang 2022) — Paper Notes|Childs-Leng-Li-Liu-Zhang (2022)]] — tight complexity for first-quantized real-space simulation: product formulas achieve $(Nt)^{1+o(1)}$, qubitization costs $O(N^2 t)$; subnormalization gap from kinetic energy spectrum; near-tight $\Omega(Nt)$ lower bound
...
### Open quantum systems / Lindbladian simulation
- [[Efficient Simulation of Sparse Markovian Quantum Dynamics (Childs-Li 2017) — Paper Notes|Childs-Li (2017)]] — first efficient quantum algorithm for sparse Lindblad simulation; vectorisation + Stinespring dilation + quantum walk; $\tilde{O}(d^2 K \Lambda^{1/2} t^{1/2}/\varepsilon^{1/2})$ query complexity
...
- [[Exponential Quantum Advantage in Processing Massive Classical Data (Zhao, Zlokapa, Neven et al 2026) — Paper Notes|Zhao, Zlokapa, Neven et al. (2026)]] — streaming-data model for linear systems / classification / PCA without qRAM; introduces quantum oracle sketching and interferometric classical shadow; proves bounded-space classical hardness from oracle separations
...
- [[Quantum Algorithms for Systems of Linear Equations Inspired by Adiabatic Quantum Computing (Subaşı-Somma-Orsucci 2018) — Paper Notes|Subaşı-Somma-Orsucci (2018/2019)]] — first competitive adiabatic-style QLSA; randomized eigenpath traversal with spectral-gap amplification gives $O(\kappa\log\kappa/\varepsilon)$
...
- [[Quantum Lower Bounds for Simulating Fluid Dynamics (Ameri-Carolan-Childs-Krovi 2026) — Paper Notes|Ameri-Carolan-Childs-Krovi (2026)]] — copy-complexity lower bounds: $\Omega(T^2)$ for KdV equation, $e^{\Omega(T)}$ for incompressible Euler; polynomial-method argument applied to state copies; rules out general-purpose quantum speedup for nonlinear fluid simulation
...
- [[Fast Quantum Algorithm for Differential Equations (Bagherimehrab-Nakaji-Wiebe-Brennen-Sanders-Aspuru-Guzik 2023) — Paper Notes|Bagherimehrab-Nakaji-Wiebe-Brennen-Sanders-Aspuru-Guzik (2023)]] — wavelet preconditioning for elliptic PDEs; makes the effective QLSA condition number $O(1)$ and prepares an observable-ready solution state in $O(\log(1/\varepsilon))$ queries and $O(dn^2)$ gates
...
## Quantum Thermodynamics / Cooling

- [[Novel Technique for Optimal and Robust Algorithmic Cooling (Raeisi-Kieferová-Mosca 2019) — Paper Notes|Raeisi-Kieferová-Mosca (2019)]] — two-sort algorithmic cooling: fixed adjacent-swap compression reaches the same asymptotic HBAC limit as partner pairing, without adaptive state-dependent sorting
...
- [[Learning quantum states prepared by shallow circuits in polynomial time (Landau-Liu 2024) — Paper Notes|Landau-Liu (2024)]] — reconstructs shallow lattice-prepared states from local reduced density matrices via replacement processes and covering schemes; cleaner state-learning result than the earlier 2D band-slicing approach.
...
- [[Predicting Many Properties from Very Few Measurements (Huang-Kueng-Preskill 2020) — Paper Notes|Huang-Kueng-Preskill (2020)]] — classical shadows: randomized single-copy measurements + channel inversion estimate $M$ observables with $O(\|O\|_{\rm shadow}^2\log M/\varepsilon^2)$ copies; foundational for Robert Huang's later QML and many-body learning work.
- [[Efficient estimation of Pauli observables by derandomization (Huang-Kueng-Preskill 2021) — Paper Notes|Huang-Kueng-Preskill (2021, derandomization)]] — deterministic local-Pauli measurement design via conditional expectations; fixes the worst practical failure mode of randomized local shadows for VQE-style observable families.
- [[Learning to predict arbitrary quantum processes (Huang-Chen-Preskill 2023) — Paper Notes|Huang-Chen-Preskill (2023)]] — learns bounded-degree output properties of an arbitrary unknown process from randomized product-state experiments by estimating a low-degree Heisenberg-picture surrogate.
- [[Learning shallow quantum circuits (Huang-Liu-Broughton-Kim-Anshu-Landau-McClean 2024) — Paper Notes|Huang-Liu-Broughton-Kim-Anshu-Landau-McClean (2024)]] — first poly-time classical learner for unknown constant-depth quantum circuits from single-qubit measurement data; key idea is ancilla-sewing of local inversions.
- [[Predicting quantum channels over general product distributions (Chen-de Dios Pont-Hsieh-Huang-Lange-Li 2024) — Paper Notes|Chen-de Dios Pont-Hsieh-Huang-Lange-Li (2024)]] — extends HCP23 from locally flat inputs to essentially any nonclassical product distribution using biased Pauli analysis and a quadratic-form comparison argument.
...
- [[Robustness of Adiabatic Quantum Computation (Childs-Farhi-Preskill 2001) — Paper Notes|Childs-Farhi-Preskill (2001)]] — Markovian noise (Lindblad/Redfield) analysis of AQC under weak environmental coupling; perturbative density matrix expansion; spectral gap as passive decoherence filter; thermalisation window and bath-assisted cooling
...
- [[Quantum Algorithms and Lower Bounds for Convex Optimization (Chakrabarti-Childs-Li-Wu 2020) — Paper Notes|Chakrabarti-Childs-Li-Wu (2020)]] — quantum subgradient $\tilde{O}(\sqrt{n}/\varepsilon)$ (tight); quantum cutting plane $\tilde{O}(n^{3/2})$ via QLSA Newton steps; lower bounds via search embedding; Gibbs sampling speedup via quantum walk
- [[Symmetries, Graph Properties, and Quantum Speedups (Ben-David-Childs-Gilyén-Kretschmer-Podder-Wang 2020) — Paper Notes|Ben-David-Childs-Gilyén-Kretschmer-Podder-Wang (2020)]] — well-shuffling reduction characterises exponential quantum speedups for graph properties; adjacency-list vs adjacency-matrix query models; constructs problems with optimal quantum-classical gaps
- [[Quantum Divide and Conquer (Childs-Kothari-Kovacs-Deak-Sundaram-Wang 2025) — Paper Notes|Childs-Kothari-Kovacs-Deak-Sundaram-Wang (2025)]] — $\sqrt{a}$ recurrence rule: replaces $a$ subproblems with $\sqrt{a}$ via span program composition; applications to regular language recognition $\tilde{O}(\sqrt{n})$, LIS, LCS, string rotation
...
- [[Quantum Algorithms for Algebraic Problems (Childs-van Dam 2010) — Paper Notes|Childs-van Dam (2010)]] — Rev. Mod. Phys. survey: HSP as unifying framework; abelian instances (factoring, discrete log, Pell, principal ideal), continuous-group algorithms, non-abelian hardness (dihedral, symmetric), graph isomorphism barrier
...
- [[Addition on a Quantum Computer (Draper 2000) — Paper Notes|Draper (2000)]] — QFT-based adder: addition via controlled phase rotations in the Fourier basis; eliminates carry qubits ($2n$ qubits vs. $3n$); $O(n^2)$ gates exact, $O(n \log n)$ approximate
- [[A New Quantum Ripple-Carry Addition Circuit (Cuccaro-Draper-Kutin-Moulton 2004) — Paper Notes|Cuccaro-Draper-Kutin-Moulton (2004)]] — single-ancilla ripple-carry adder via MAJ/UMA gates; $2n - 1$ Toffolis, depth $2n + 4$; basis for Gidney's T-count optimisation
- [[A Logarithmic-Depth Quantum Carry-Lookahead Adder (Draper-Kutin-Rains-Svore 2004) — Paper Notes|Draper-Kutin-Rains-Svore (2004)]] — first $O(\log n)$-depth carry-based quantum adder via carry-lookahead tree; $O(n)$ Toffolis and $O(n)$ ancillae; in-place, modular, and comparator variants
- [[Halving the Cost of Quantum Addition (Gidney 2018) — Paper Notes|Gidney (2018)]] — temporary logical-AND primitive halves T-count of paired Toffoli gates; $n$-bit adder in $4n + O(1)$ T gates (down from $8n$); widely applicable to adders, Grover oracles, multi-controlled gates, and arithmetic subroutines
- [[Approximate Encoded Permutations and Piecewise Quantum Adders (Gidney 2019) — Paper Notes|Gidney (2019)]] — approximate encoded permutation framework; oblivious carry runways for piecewise addition in $O(s+m)$ depth; coset representation turns modular addition into non-modular; lowest spacetime volume at Shor-relevant sizes
- [[A Classical-Quantum Adder with Constant Workspace and Linear Gates (Gidney 2025) — Paper Notes|Gidney (2025)]] — closes 21-year open problem: CQ adder with $4n \pm O(1)$ Toffolis and 3 clean ancillae; carry venting via X-basis measurement + half-register dirty workspace borrowing
- [[Improved Reversible and Quantum Circuits for Karatsuba-Based Integer Multiplication (Parent-Roetteler-Mosca 2017) — Paper Notes|Parent-Roetteler-Mosca (2017)]] — Karatsuba multiplier with space reduced from $O(n^{1.585})$ to $O(n^{1.427})$ via pebble games on ternary trees; $O(n^{\log_2 3})$ Toffoli gates; adaptive cutoff optimisation
- [[Factoring using 2n+2 qubits with Toffoli based modular multiplication (Häner-Roetteler-Svore 2017) — Paper Notes|Häner-Roetteler-Svore (2017)]] — Shor's algorithm in $2n + 2$ qubits with purely Toffoli-based modular multiplication; dirty-ancilla constant adder in $O(n \log n)$; avoids rotation synthesis overhead
- [[Optimizing Quantum Circuits for Arithmetic (Häner-Roetteler-Svore 2018) — Paper Notes|Häner-Roetteler-Svore (2018)]] — gate-level circuits for evaluating smooth functions (Gaussian, tanh, sin/cos, exp, arcsin, $1/\sqrt{x}$) via parallel piecewise polynomial evaluation and reversible Newton iteration; automatic oracle compilation via Remez algorithm; concrete Toffoli counts for quantum arithmetic subroutines
- [[A Log-Depth In-Place Quantum Fourier Transform (Kahanamoku-Meyer-Blue-Bergamaschi-Gidney-Chuang 2025) — Paper Notes|Kahanamoku-Meyer, Blue, Bergamaschi, Gidney, Chuang (2025)]] — optimistic QFT in $O(\log(n/\varepsilon))$ depth with zero ancillae and nearest-neighbour gates; worst-to-average-case reduction yields first optimal-depth approximate QFT with sublinear ancillae; factoring in depth $O(n^{1+\epsilon})$ with $2n + O(n/\log n)$ qubits
...
- [[Information-Theoretic Bounds on Quantum Advantage in ML (Huang-Kueng-Preskill 2021) — Paper Notes|Huang-Kueng-Preskill (2021)]] — query-complexity limits on QML advantage: average-case prediction has at most polynomial quantum advantage over restricted classical learning, while worst-case Pauli prediction gives an exponential quantum-memory separation.
- [[Provably efficient machine learning for quantum many-body problems (Huang-Kueng-Torlai-Albert-Preskill 2022) — Paper Notes|Huang-Kueng-Torlai-Albert-Preskill (2022)]] — shadow-based classical ML for gapped many-body systems: Fourier interpolation for local ground-state observables and nonlinear shadow kernels for phase classification.
- [[Learning to predict arbitrary quantum processes (Huang-Chen-Preskill 2023) — Paper Notes|Huang-Chen-Preskill (2023)]] — strongest version of the “power of data” story in this cluster: even exponentially complex dynamics can be classically predictable for locally flat inputs and bounded-degree observables.
- [[Learning shallow quantum circuits (Huang-Liu-Broughton-Kim-Anshu-Landau-McClean 2024) — Paper Notes|Huang-Liu-Broughton-Kim-Anshu-Landau-McClean (2024)]] — separates learnability from simulability for constant-depth circuits; full circuit description becomes classically learnable from shadow-style data despite sampling hardness.
- [[Predicting quantum channels over general product distributions (Chen-de Dios Pont-Hsieh-Huang-Lange-Li 2024) — Paper Notes|Chen-de Dios Pont-Hsieh-Huang-Lange-Li (2024)]] — biased-product-distribution extension of the HCP process-learning theorem; the real boundary is classical vs nonclassical one-qubit marginals, not local-flatness symmetry.
- [[Exponential Quantum Advantage in Processing Massive Classical Data (Zhao, Zlokapa, Neven et al 2026) — Paper Notes|Zhao, Zlokapa, Neven et al. (2026)]] — bounded-space quantum processing of massive classical data streams; no qRAM assumption; reduces query-complexity separations to space/sample separations for LS-SVM classification, PCA-style dimension reduction, and linear systems
...
- [[Predicting Many Properties from Very Few Measurements (Huang-Kueng-Preskill 2020) — Paper Notes|Huang-Kueng-Preskill (2020)]] — classical-shadows foundation: fixed randomized measurements, inverse channel snapshots, and median-of-means prediction; $O(4^k\log M/\varepsilon^2)$ copies for $k$-local observables with local Pauli shadows.
- [[Efficient estimation of Pauli observables by derandomization (Huang-Kueng-Preskill 2021) — Paper Notes|Huang-Kueng-Preskill (2021, derandomization)]] — deterministic replacement for random local Pauli settings; especially useful when a few high-weight Pauli strings would otherwise dominate the shot count.
