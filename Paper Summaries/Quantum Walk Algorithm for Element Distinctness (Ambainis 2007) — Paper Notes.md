...
Gives an $O(N^{2/3})$ quantum query algorithm for element distinctness, matching Shi's polynomial-method lower bound. The technique — quantum walk on the Johnson graph — became the template for a whole family of quantum algorithms. Also gives $O(N^{k/(k+1)})$ for $k$-distinctness.
...
**The fix:** Reuse data between adjacent vertices. Moving from $v_S$ to $v_T$ (differing in one element) costs only constant queries. The quantum walk amortises the setup cost.
...
**Queries per step:** constant, with one query to load $x_y$ and one unquery to erase it in this reversible implementation. The asymptotic analysis suppresses this constant.
...
Setting $t_2 = \lceil \frac{\pi}{3\sqrt{k}} \sqrt{r} \rceil$ makes all non-trivial eigenvalues $e^{i\theta}$ with $\theta \in [c, 2\pi - c]$ for a constant $c$ — bounded away from 1. The arXiv v9 notes sign corrections on the eigenvalue pages, so these formulas should be treated as analysis-level structure, not copied as implementation-ready circuit data.
...
- [[Quantum Algorithm for k-Distinctness with Prior Knowledge on the Input (Belovs-Lee 2011) — Paper Notes|Belovs-Lee (2011)]] — improves the $k>2$ exponent to $O\!\left(n^{1 - 2^{k-2}/(2^k-1)}\right)$ under a prior-knowledge promise, using learning graphs instead of a Johnson-graph walk
- **Childs, Eisenberg (2005):** Alternative analysis showing success probability $1 - o(1)$

The walk algorithm and the lower-bound method are separate tools: this paper is an upper-bound construction, while the tight element-distinctness lower bound comes from Shi's polynomial-method analysis rather than Ambainis's adversary method.

---

## Reusable ideas

1. **[[Walk on the Johnson Graph for Subset Search]]:** Walk on subsets of $[N]$, storing queried values. Moving between adjacent subsets costs constant queries; checking if marked costs 0 queries once the data are stored. Balances setup cost $r$ against walk cost $N^{k/2}/r^{(k-1)/2}$ for Ambainis-style $k$-distinctness.
...
- Shi (2002) — $\Omega(N^{2/3})$ lower bound via polynomial method
...
### Paper notes
- [[Quantum Lower Bounds by Quantum Arguments (Ambainis 2000) — Paper Notes]] — adversary-method background; not the source of the tight element-distinctness lower bound
- [[Quantum Lower Bounds by Polynomials (Beals-Buhrman-Cleve-Mosca-de Wolf 1998) — Paper Notes]] — polynomial-method background; Shi's $\Omega(N^{2/3})$ lower bound uses polynomial degree arguments
...
- [[Search via Quantum Walk (Magniez-Nayak-Roland-Santha 2007) — Paper Notes]] — generalises this approach to ergodic reversible chains, with a non-reversible extension via discriminant singular values; handles multiple solutions without reduction
- [[Quantum Algorithm for k-Distinctness with Prior Knowledge on the Input (Belovs-Lee 2011) — Paper Notes]] — learning-graph successor for promised $k$-distinctness
