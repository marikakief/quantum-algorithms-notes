...
**Modern amplitude-estimation interpretation:** Given a quantum algorithm $A$ with $A|0\rangle = |\Psi\rangle$ and success probability $a = \langle\Psi_a|\Psi_a\rangle$, estimate $a$. In this 1998 paper the named task is quantum counting; the fully general "amplitude estimation" terminology is associated more strongly with the later BHMT treatment.
...
2. **Quantum speedup of classical heuristics** — if a coherently implementable classical heuristic finds a solution in expected time $T$, with a checkable success predicate and reversible implementation costs included, a quantum version finds one in $O(\sqrt{T})$. This uses Cauchy-Schwarz on the heuristic's success distribution.
...
**Exact amplification:** The paper includes arbitrary-phase variants that can achieve success probability exactly 1 when the relevant success amplitude/angle is known:
...
This is the formal justification for a broad class of quadratic heuristic speedups. It applies to Las Vegas algorithms, randomised restarts, and local search heuristics when the randomness can be put in superposition, the computation and stopping/checking logic can be made reversible, and the oracle/checking cost is included.
...
| Exact count | Two-phase estimate | $O(\sqrt{tN})$ away from endpoints; endpoint-safe statements need separate handling | $\tilde{t} = t$ w.h.p. |
...
For exact counting, a two-phase approach: first estimate $t$ to within $O(\sqrt{t})$ using Count($F, \sqrt{N}$), then use this to set $P \approx 20\sqrt{\tilde{t}N}$ for a second call that resolves $t$ exactly (error $< 1/2$, round to nearest integer). The displayed $O(\sqrt{tN})$ scaling is the useful non-endpoint form; exact counting statements require care at $t=0$ and $t=N$, where separate endpoint-safe decision/checking behavior is needed. The polynomial method gives matching lower-bound behavior in the appropriate regimes.
...
| Exact counting | endpoint-sensitive; often written $O(\sqrt{tN})$ away from $t=0,N$ | Optimal in the corresponding regimes |
...
| [[Quantum Amplitude Amplification and Estimation (Brassard-Høyer-Mosca-Tapp 2002) — Paper Notes\|BHMT (2002)]] | Definitive treatment with improved error analysis |

---

## Limits / caveats

- The $8/\pi^2$ success probability is for a single phase-estimation/counting run. Median or majority repetition raises confidence to $1-\delta$ at $O(\log(1/\delta))$ overhead.
...
- The exact counting algorithm has endpoint-sensitive query complexity. The compact $O(\sqrt{tN})$ expression should not be read as covering $t=0$ or $t=N$ without the separate endpoint handling and two-phase setup.
...
2. **[[Quadratic Speedup for Classical Heuristics via Amplitude Amplification]]:** A fixed-runtime or appropriately bucketed classical randomized search heuristic with coherent random seeds, reversible implementation, and a checkable success predicate can be sped up to $O(\sqrt{T})$ by treating the heuristic as the "preparation" algorithm $A$ in amplitude amplification. The proof via Cauchy-Schwarz on the success distribution is clean, but the oracle/circuit model assumptions are part of the theorem.
