# FeMoCo Resource Estimation Timeline
...
|------|-------|--------|-------------|---------------|----------------|-------|
| 2017 | Reiher, Wiebe, Svore, Wecker, Troyer | Trotter (product formula) | (54e, 54o) / 108 SO | ~5 × 10¹³ | 111 | The original. Loose Trotter error bounds. |
...
4. **The error budget is tight.** Chemical accuracy (1.6 mHa) requires careful partitioning between Hamiltonian approximation error (from DFTHC truncation) and phase estimation statistical error. The randomized error budget trick from Low et al. (2025) helps, but any individual run could be unlucky.
