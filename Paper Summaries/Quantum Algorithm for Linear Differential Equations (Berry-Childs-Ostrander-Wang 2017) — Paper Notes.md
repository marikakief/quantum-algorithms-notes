...
- [[Quantum Algorithm for Time-Dependent Differential Equations Using Dyson Series (Berry-Costa 2022) — Paper Notes]] — extends this paper's approach to time-dependent coefficients using Dyson series block-encoding; eliminates diagonalisability requirement via logarithmic norm
- [[Further Improving Quantum Algorithms for Nonlinear DEs via Higher-Order Methods and Rescaling (Costa-Schleich-Morales-Berry 2023) — Paper Notes]] — uses the Taylor-series ODE solver (via Berry-Costa 2022) as a subroutine for nonlinear ODEs after Carleman linearisation
- [[High-Order Quantum Algorithm for Solving Linear Differential Equations (Berry 2014) — Paper Notes]] — direct predecessor; this paper delivers the exponential precision improvement Berry 2014 lacked
- [[Quantum Algorithm for Linear Systems of Equations (Harrow-Hassidim-Lloyd 2009) — Paper Notes]] — HHL; the QLSP solver used as subroutine here, improved in the downstream QLSP lineage
- [[Optimal Scaling Quantum Linear Systems Solver via Discrete Adiabatic Theorem (Costa, An, Sanders, Su, Babbush, Berry 2021) — Paper Notes]] — achieves the optimal QLSP complexity; the definitive improvement on the HHL subroutine used in this paper
- [[The Discrete Adiabatic QLSP Has Lower Constant Factors than the Randomized Solver (Costa, An, Babbush, Berry 2023) — Paper Notes]] — numerical validation of the practical efficiency of the optimal QLSP successor
- [[Large Time-Step Discretisation of Adiabatic Quantum Dynamics (An-Costa-Berry 2025) — Paper Notes]] — further generalises the adiabatic simulation approach in the same lineage; $\varepsilon$-independent time steps
- [[Time-Dependent Hamiltonian Simulation via Dyson Series (Kieferová-Scherer-Berry 2018) — Paper Notes]] — extends related LCU-based simulation to time-dependent Hamiltonians; the Dyson approach Berry-Costa 2022 uses to extend this paper's work
