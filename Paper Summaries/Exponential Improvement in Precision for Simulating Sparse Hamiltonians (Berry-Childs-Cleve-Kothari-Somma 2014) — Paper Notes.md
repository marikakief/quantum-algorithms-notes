...
$$|0\rangle|\psi\rangle \mapsto \sqrt{q_\alpha}\;|0\rangle Q^\alpha|\psi\rangle + \sqrt{1-q_\alpha}\;|1\rangle|\phi\rangle$$

where $q_\alpha = 1/(1 + \sin(\pi\alpha))$ and, up to a global phase convention, $Q^\alpha$ is the fractional query generated by the self-inverse query $Q$.
...
1. Decompose $H$ into $d^2$ 1-sparse Hamiltonians (Lemma 4.4, with a bipartite trick that removes the $\log^* n$ factor); this $d^2$ decomposition is the source of the remaining quadratic sparsity dependence
...
For constant $t$, this overlap is super-exponentially small in $N$ (on the order of $N^{-N}$). Inverting this scale gives $N = \Theta(\log(1/\varepsilon)/\log\log(1/\varepsilon))$, which is the parameter regime used in the lower bound.
...
1. **OAA.** This single idea — amplitude amplification without knowing the input state — has become one of the central tools in LCU-based simulation and block-encoding constructions. With the QSVT lens it is one important polynomial-transformation primitive, but not every modern quantum algorithm uses OAA.
...
- [[Optimal Hamiltonian Simulation by QSP (Low-Chuang 2016-2017) — Paper Notes]] — achieves additive time/precision scaling $O(\tau + \log(1/\varepsilon)/\log\log(1/\varepsilon))$ in the normalized query model
