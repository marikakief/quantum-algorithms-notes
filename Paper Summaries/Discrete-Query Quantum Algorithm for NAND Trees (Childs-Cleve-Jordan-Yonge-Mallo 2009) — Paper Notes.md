...
The result is short and clean — essentially a two-page observation that high-order product formula simulation of the FGG Hamiltonian gives the required query complexity. It shows the FGG Hamiltonian-oracle approach can be discretised with subpolynomial overhead; the contemporaneous ACRŠZ formula-walk work gives the optimal $O(\sqrt{N})$ query bound for balanced and approximately balanced NAND trees.
...
The paper observes that two standard queries (Eq. 3: $U_O|k, a\rangle = |k, a \oplus x_k\rangle$) suffice to simulate evolution by this self-inverse Hamiltonian oracle $H_O$ for arbitrary time $t$:
...
For any target $\varepsilon>0$, choose a fixed Suzuki order $p$ large enough that the exponent $1 + 1/(2p)$ is close to 1. Since $T = O(\sqrt{N\log N})$ and each step uses 2 queries to $U_O$, the total query count is:
...
- The Hamiltonian-to-discrete conversion here is specific to this standard self-inverse oracle Hamiltonian plus a time-independent driving term. It does *not* give a general conversion for arbitrary continuous-time query algorithms, and it is not gate-efficient by itself.

---

## Reusable ideas

1. [[Hamiltonian Oracle to Discrete Query Conversion via Product Formulas]] — Convert this kind of continuous-time Hamiltonian-oracle algorithm into a discrete-query algorithm by simulating $e^{-i(H_O + H_D)t}$ with product formulas, where each $e^{-iH_O \Delta t}$ step costs a constant number of standard queries.
