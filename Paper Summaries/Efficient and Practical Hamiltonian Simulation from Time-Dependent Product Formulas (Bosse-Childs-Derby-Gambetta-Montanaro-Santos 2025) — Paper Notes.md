...
The base result (**THRIFT**): a first-order product formula with error $O(\alpha^2 t^2)$ instead of $O(\alpha t^2)$ — a factor $\alpha$ improvement. Higher-order THRIFT via Suzuki recursion gives $O(\alpha^2 t^{k+1})$, and the paper proves this $\alpha^2$ scaling is optimal within its time-ordered-product-formula model.
...
Each factor $e^{itH_0} e^{-it(H_0 + \alpha H_1^\gamma)}$ implements the time-ordered evolution under the individual interaction-picture term $\alpha H_1^\gamma(\tau)$, provided one can implement evolution under $H_0+\alpha H_1^\gamma$. The formula is just a standard first-order Trotter splitting of $H = H_0 + \alpha \sum_\gamma H_1^\gamma$ using the unusual summands $\{H_0 + \alpha H_1^\gamma\}$ and $\{-H_0\}$.
...
where $A_0 = \alpha H_1$ and $A_{j+1}$ is defined recursively from commutators of $A_j$. Error: $O(\alpha^{2^k} t^{2^{k+1}-1})$ if the exponentials are implemented exactly. This exact-implementation premise is realistic only when the Fer-generated operators have special structure; otherwise those exponentials must themselves be approximated, giving the same practical scaling as Magnus-THRIFT.
...
**Theorem 10 (Optimality):** $\alpha^2$ scaling is optimal among all product formulas built from time-ordered evolutions of Hamiltonian terms.

**Numerical headlines under the paper's benchmark and gate-count assumptions:**
...
| Corrected product formulas ([[Faster Algorithmic Quantum and Classical Simulations by Corrected Product Formulas (Bagherimehrab-Berry-Schleich-Aldossary-Angulo-Aspuru-Guzik 2024) — Paper Notes\|Bagherimehrab et al. 2024]]) | $O(\alpha^2 t^{2k+1})$ | No | Same $\alpha^2$ scaling through boundary commutator correctors; uses $e^{At}, e^{Bt}$ separately |
...
The key practical insight: THRIFT has the *same circuit depth per step* as standard Trotter when $H_0$ is 1-local (so $H_1(t)$ preserves locality), but with better error. For models where the interaction picture increases locality (Fermi-Hubbard in the paper's benchmarks), the advantage is lost to higher per-step costs. CPF reaches the same $\alpha^2$ scaling by a different primitive: compiled correctors around ordinary $A$ and $B$ evolutions rather than time-ordered evolutions of interaction-picture terms.

## Limits / caveats

- The advantage requires $e^{-itH_0}$ and the required $e^{-it(H_0+\alpha H_1^\gamma)}$-type evolutions to be implementable cheaply enough that their cost does not erase the improved error scaling. This is plausible when $H_0$ is diagonal or single-qubit and each added term preserves easy structure, but not for general $H_0$.
...
- Magnus-THRIFT 2 is only competitive for $\alpha < 10^{-2}$ in the paper's numerical model. The Magnus terms involve commutators that may require more complex circuits.
...
2. **[[Magnus Expansion for Time-Ordered Product Formulas]]** — approximate the time-ordered exponential via the Magnus expansion $e^{\Omega^{[k]}}$, where the Magnus terms are nested commutator integrals. Achieves $O((\alpha t)^{k+1})$ error, breaking the $\alpha^2$ barrier of plain THRIFT when the resulting terms can be implemented efficiently.
