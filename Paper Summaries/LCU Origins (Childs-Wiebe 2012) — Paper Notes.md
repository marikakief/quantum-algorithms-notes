...
$$
an improvement over prior product-formula bounds (which had exponent constant ≈2.06 or ≈2.54). The exponent is subexponential in the precision parameter — much better than [[Product Formulas]]s but not the polylog(1/ε) that later Taylor/QSP methods achieve. The sign problem (negative LCU coefficients) is the obstacle.
...
| application to multi-[[Product Formulas]]s | concrete simulation payoff |
...
3. **[[Coefficient Engineering for Subtractive LCUs]]** — choose the multi-[[Product Formulas]] coefficients $\ell_q$ so the dominant positive term is large enough to suppress subtraction failure probability. The first systematic approach to managing signed LCU coefficients.
...
- The multi-[[Product Formulas]] approach is sensitive to coefficient engineering: choosing the Vandermonde parameters poorly makes the subtraction step nearly always fail.
