...
Most relevant when:
- [[product formula]]s are otherwise attractive (easy compilation, no appetite for block-encoding machinery),
- the main bottleneck is sensitivity to ordering in large-$L$ decompositions,
- moderate precision suffices.

For high precision or adversarial structure, [[product formula]]s are still not the right hammer regardless of randomization.

## Predecessors

Random-order heuristics for [[product formula]]s appeared earlier in numerical ODE literature (see Süli, Iserles and related work on splitting methods). This paper provides rigorous quantum information guarantees.
...
- [[Randomizing Multi-Product Formulas for Hamiltonian Simulation (Faehrmann-Steudtner-Kueng-Kieferová-Eisert 2022) — Paper Notes]] — combines multi-[[product formula]]s with randomized sampling; complementary approach to the permutation-averaging technique here
