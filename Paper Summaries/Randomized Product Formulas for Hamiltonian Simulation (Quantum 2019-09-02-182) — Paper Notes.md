...
Shows that randomizing the ordering of Hamiltonian terms in a [[Product Formulas]] gives provably stronger error bounds than any fixed ordering, sometimes asymptotically better than previous commutator-based bounds despite using less information about the Hamiltonian structure.
...
For $H = \sum_{j=1}^L H_j$, a deterministic [[Product Formulas]] fixes one permutation. This paper instead draws a fresh random permutation $\sigma$ for each segment and analyzes the average channel over these random circuits.
...
1. Expand the [[Product Formulas]] error in a Taylor series.
...
This is not the same as qDRIFT, which samples single terms by weight. Here the basic unit is a full shuffled [[Product Formulas]], not one random exponential.

| Method | What randomness does | Basic unit |
|---|---|---|
| qDRIFT | Samples single term by weight | One random exponential |
| This paper | Randomizes the term ordering | A full shuffled [[Product Formulas]] |
...
Most relevant when:
- [[Product Formulas]]s are otherwise attractive (easy compilation, no appetite for block-encoding machinery),
- the main bottleneck is sensitivity to ordering in large-$L$ decompositions,
- moderate precision suffices.

For high precision or adversarial structure, [[Product Formulas]]s are still not the right hammer regardless of randomization.

## Predecessors

Random-order heuristics for [[Product Formulas]]s appeared earlier in numerical ODE literature (see Süli, Iserles and related work on splitting methods). This paper provides rigorous quantum information guarantees.
...
- [[Quantum Simulation of Time-Dependent Hamiltonians and the Convenient Illusion of Hilbert Space (Poulin-Qarry-Somma-Verstraete 2011) — Paper Notes]] — earlier randomised [[Product Formulas]] for time-dependent Hamiltonians via Monte Carlo time-sampling
...
- [[Randomizing Multi-Product Formulas for Hamiltonian Simulation (Faehrmann-Steudtner-Kueng-Kieferová-Eisert 2022) — Paper Notes]] — combines multi-[[Product Formulas]]s with randomized sampling; complementary approach to the permutation-averaging technique here
