...
Introduces **corrected [[Product Formulas]]s (CPFs)** — standard [[Product Formulas]]s with auxiliary "corrector" terms $e^{\pm C}$ injected at the boundaries. The correctors are exponentials of nested commutators $\mathrm{ad}_A^j(B)$ that systematically cancel leading error terms in the [[Finite Nested-Commutator Expansion|BCH kernel]] of the product formula.
...
All correctors modify the kernel $K$ of a standard [[Product Formulas]] $S(λ) = e^K$ to a better kernel $K'$ closer to the exact kernel $λH$.
...
| THRIFT (Bosse et al., 2024 arXiv / 2025 journal) | $O(\alpha^2 t^{2k+1})$ | Yes | Multiplicative factor; needs access to evolutions under $A + \alpha H_j$-type pieces |
...
At the primitive level, CPF and THRIFT improve the same perturbed setting in different ways. CPF stays in the original frame and injects compiled boundary commutator correctors using $e^{At}$ and $e^{Bt}$. THRIFT moves to the interaction picture and assumes the ability to implement time-ordered evolutions generated by individual interaction-picture terms, equivalently $A+\alpha H_j$-type pieces in the time-independent construction.
...
6. **Concurrent work.** THRIFT (Bosse et al., 2024 arXiv / 2025 journal) achieves the same $O(\alpha^2 t^{2k+1})$ bound via interaction-frame [[Product Formulas]]s. Bosse et al. prove $\alpha^2$ optimality for the class of product formulas built from time-ordered evolutions of Hamiltonian terms, not for arbitrary Hamiltonian-simulation algorithms.
...
The practical impact will be largest for **early fault-tolerant simulation of perturbed lattice models** where $H=A+\alpha B$ has exactly simulatable pieces and $\alpha \ll 1$. For such systems, the factor-of-$\alpha$ improvement in the error bound can translate directly to fewer Trotter steps, and the symplectic corrector adds negligible overhead. Specific chemistry or Hubbard examples require checking that the chosen partition really has this structure and that corrector compilation does not erase the gain.
...
1. [[Symplectic Corrector Injection for Product Formulas]] — the core technique: conjugate a [[Product Formulas]] by $e^{\pm C}$ where $C$ is a linear combination of nested commutators, to cancel leading BCH error terms at negligible additive cost
...
- Bosse, Childs, Derby, Gambetta, Montanaro, Santos (2024 arXiv / 2025 journal) — THRIFT algorithm; same $\alpha^2$ error scaling via interaction-frame product formulas; proved $\alpha^2$ optimality within the time-ordered-product-formula model
...
- [[Selection and Improvement of Product Formulae for Best Performance of Quantum Simulation (Morales-Costa-Pantaleoni-Burgarth-Sanders-Berry 2025) — Paper Notes]] — finds over-parameterized 8th-order [[Product Formulas]]s ~100× more accurate; the corrector approach here and the coefficient-optimization approach there are complementary strategies for reducing product-formula error constants
