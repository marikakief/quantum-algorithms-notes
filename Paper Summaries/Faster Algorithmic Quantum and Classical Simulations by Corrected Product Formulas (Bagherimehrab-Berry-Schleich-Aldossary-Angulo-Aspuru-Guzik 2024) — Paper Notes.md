...
6. **Concurrent work.** THRIFT (Bosse et al., arXiv:2403.08729) achieves the same $O(\alpha^2 t^{2k+1})$ bound via interaction-frame [[Product Formulas]]s. The $\alpha^2$ scaling is provably optimal for product-formula-type approaches (shown by Bosse et al.).
