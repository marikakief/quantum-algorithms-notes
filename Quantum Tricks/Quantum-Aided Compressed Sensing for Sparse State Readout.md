...
## What it does
Recovers a sparse classical description of a quantum state with cost $\tilde{O}(M'^2)$ state preparations rather than $O(M)$ full tomography, when the state has effective support on only $M'$ out of $M$ basis elements and the restricted tomography problem satisfies the usual compressed-sensing assumptions.
...
1. **Support identification.** Prepare $|\lambda\rangle$ and measure in the computational basis $O(M')$ times. The heavy components with large probabilities $|\lambda_j|^2$ show up frequently. This identifies the support $S$ of size $M'$ with high probability when the support probabilities are well separated; it does not directly reveal amplitudes, phases, or small signed components.
...
3. **Compressed-sensing tomography.** Apply standard compressed-sensing quantum state tomography to $|\lambda_S\rangle$, assuming the restricted state is rank/sparsity structured and measured with an appropriate ensemble:
...
The key saving: $M'^2$ instead of $M$ (or worse, $2^n$ for $n$-qubit states in general), which is only exponential when $M'$ is polylogarithmic or otherwise much smaller than $M$.
...
Compare to full tomography: $O(M/\varepsilon^2)$ — exponentially better only when $M' = O(\mathrm{polylog}\, M)$ or similarly tiny compared with $M$.
...
- Compressed-sensing tomography requires appropriate measurement bases (e.g., random Pauli settings) and rank/sparsity promises; this may not always be physically convenient.
