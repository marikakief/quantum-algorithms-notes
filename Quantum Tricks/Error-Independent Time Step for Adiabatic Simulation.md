...
**Simplified $p$-th order [[Product Formulas]]:** Set
...
- The [[Product Formulas]] must be the "simplified" version (scheduling function evaluated at a single time per step). Full higher-order Trotter-Suzuki with staggered time evaluations isn't covered by the current analysis.
