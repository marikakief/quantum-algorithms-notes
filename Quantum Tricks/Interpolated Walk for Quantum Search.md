...
The [[Quantum Speed-Up of Markov Chain Based Algorithms (Szegedy 2004) — Paper Notes|Szegedy walk operator]] $W(s)$ of $P(s)$ has a unique $+1$ eigenvector $|v_n(s)\rangle$ in the span of $|U\rangle$ and $|M\rangle$, the stationary-weighted unmarked and marked superpositions:
...
to balance the overlaps, since $(1-s)(1-p_M)=p_M$ gives $\cos\theta = \sin\theta = 1/\sqrt{2}$. Then [[Phase Estimation as Eigenvalue Filter for Walk-Based Search|eigenvalue estimation]] on $W(s^*)$ with precision $T = O(\sqrt{\mathrm{HT}^+(P,M)})$ projects the initial state $|U\rangle|\bar{0}\rangle$ onto $|v_n(s^*)\rangle$, which has constant overlap on marked vertices. If $p_M$ is already constant, the initial setup/check step already has constant marked probability.
...
## Complexity
$S + O(\sqrt{\mathrm{HT}^+(P,M)}) \cdot (U + C)$ for known parameters. For $|M| = 1$, $\mathrm{HT}^+ = \mathrm{HT}$. When $\mathrm{HT}^+$ or $p_M$ is unknown, the paper's variants add setup repetitions/binary-search overheads, including logarithmic overhead in $S$.
