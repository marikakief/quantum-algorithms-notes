...
Establishes that the achievable Suzuki order for time-dependent Hamiltonian simulation is bounded above by the smoothness class of $H(t)$: if $H$ has only $P$ bounded derivatives, you cannot achieve error scaling better than $O(\Delta\lambda^{P+1})$ with product formulas, regardless of how deep you run the Suzuki recursion. The recursion saturates at order $P$.
...
This is a necessary condition, not just sufficient. The paper does not prove tight lower bounds on what product formulas can achieve for non-smooth $H$ — just that the Suzuki recursion mechanism breaks. Other approximation methods (Dyson series, Magnus expansion) may perform differently under the same smoothness constraint.
