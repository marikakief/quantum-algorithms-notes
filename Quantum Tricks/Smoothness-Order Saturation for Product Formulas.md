...
Establishes that the achievable Suzuki order for time-dependent [[Hamiltonian simulation]] is bounded above by the smoothness class of $H(t)$: if $H$ has only $P$ bounded derivatives, you cannot achieve error scaling better than $O(\Delta\lambda^{P+1})$ with [[product formula]]s, regardless of how deep you run the Suzuki recursion. The recursion saturates at order $P$.

## The trick

The error of the $k$-th order [[product formula]] $U_k$ is $O(\Delta\lambda^{2k+1})$ only if $H(u)$ is $2k$-smooth (i.e., $\|H^{(p)}(u)\|$ is bounded for $p = 0, \ldots, 2k$). If $H$ has only $P < 2k$ bounded derivatives, the Taylor expansion of the ordered exponential truncates at order $P$, and the cancellation mechanism in the Suzuki recursion that eliminates leading-order error terms fails to produce the expected improvement.
...
This is a necessary condition, not just sufficient. The paper does not prove tight lower bounds on what [[product formula]]s can achieve for non-smooth $H$ — just that the Suzuki recursion mechanism breaks. Other approximation methods (Dyson series, Magnus expansion) may perform differently under the same smoothness constraint.
