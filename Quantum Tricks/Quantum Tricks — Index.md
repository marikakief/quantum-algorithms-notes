# Quantum Tricks — Index

> **Purpose:** Second-brain library of advanced, reusable quantum algorithmic primitives (beyond standard QFT/Grover).
> **Search:** via qmd (collection: `obsidian`) and Obsidian search.

## How to query quickly (qmd)

```bash
qmd search "Chebyshev spectral filter"
qmd search "Dicke state inequality"
qmd query "low-energy phase estimation trick sqrt Delta lambda"
```

## Trick cards

### From arXiv:2505.01528 (SOSSA)
- [[SOS Spectral Amplification]]
- [[Double Factorisation for Block-Encoding]]
- [[Gapped Phase Estimation]]

### From arXiv:2510.04929 (Quantum Hermite Transform)
- [[QHO Fast-Forwarding via Lie Algebra]]
- [[Plancherel-Rotach Asymptotic State Preparation]]
- [[Fixed-Point Amplitude Amplification with Eigenstate Filtering]]

### From Babbush et al. TDA (arXiv:2209.13581)
- [[Dicke State Preparation via Inequality Testing]]
- [[Kaiser-Window Amplitude Estimation]]
- [[Chebyshev Polynomial Spectral Projector]]
- [[Qubitization of Block-Encoded Dirac Operators]]
- [[Fermionic-to-Qubit Mapping for Simplicial Dirac Operators]]

### From Quantum FMM (arXiv:2510.07380)
- [[Quantum Sorting for Fixed-Location Data Access]]
- [[Forward-Backward Sweep for Hierarchical Aggregation]]
- [[Per-Particle Box Registers (Avoiding Quantum Addressing)]]
- [[QROM + Newton Iteration for Function Evaluation]]
- [[Restricted Interaction Norms for Product Formulas]]

### From Shadow Hamiltonian Simulation (Nature Comms 2025)
- [[Shadow State Compression via Invariance Property]]
- [[Operator-to-State Mapping for Heisenberg Evolution]]
- [[Two-Time Correlator Encoding in Shadow States]]

### From Quantum Linear Matrix Equations (arXiv:2508.02822)
- [[Sandwich LCU for Matrix Equations]]
- [[Hubbard-Stratonovich Decoupling]]
- [[LCHS Kernel for Non-Unitary Dynamics]]

### From Hamiltonian Simulation via Taylor Series (arXiv:1312.1414 + arXiv:1412.4687)
- [[Linear Combination of Unitaries (LCU)]]
- [[Oblivious Amplitude Amplification (Robust)|Oblivious Amplitude Amplification (OAA)]]
- [[Taylor Series Truncation with ln2 Segmentation]]
- [[Unary Encoding for Sequential Controlled Operations]]
- [[Fractional-Query to Discrete-Query Reduction]]
- [[Amplitude Dilution for Exact OAA]]
- [[Near-Unitary Pairing for High-Success LCU]]
- [[Failure Branch Inversion via E(-λ)E(λ)]]
- [[Coefficient Engineering for Subtractive LCUs]]

### From fermionic eigenstate prep (Nature 2018, s41534-018-0071-5)
- [[Reversible Comparator with Swap Record]]
- [[Sorting Networks as Quantum Control-Flow Compilers]]
- [[Reverse-Sort Replay for Antisymmetrization]]
- [[Birthday-Bound Collision Filtering for Seed Superpositions]]

### From QSVT and Beyond (STOC 2019, arXiv:1806.01838)
- [[QSVT Meta-Template]]
- [[Parity-Aware Polynomial Design]]
- [[One-Ancilla Alternating Phase Sequence]]
- [[Block-Encoding Composition Algebra]]

### From Hamiltonian Simulation by Qubitization (Quantum 2019)
- [[Standard-Form Encoding (Prepare + Signal Oracle)]]
- [[Qubitization Iterate]]
- [[Invariant SU(2) Subspace Embedding]]
- [[Phased Qubitization Sequence]]

### From Optimal Hamiltonian Simulation by QSP (PRL 2017)
- [[Signal Transduction via Controlled-W]]
- [[Jacobi-Anger Truncation for QSP Simulation]]
- [[Polynomial Completion via Sum-of-Squares]]

### From Resonant Equiangular Composite Gates (PRX 2016)
- [[SU(2) Response Tuple Characterization]]
- [[Partial-Quadrature Completion via Root Grouping]]
- [[Recursive Phase Extraction from Polynomial Coefficients]]
- [[FIR-Style Minimax Design for Quantum Responses]]

### From time-dependent Dyson LCU simulation (arXiv:1805.00582)
- [[Coherent Time-Ordering via Ancilla Clocks]]
- [[Compressed Gap Encoding for Ordered Tuples]]
- [[Sorting-Based Time-Ordering for Dyson LCU]]
- [[Collision Budgeting in Discretized Ordered Integrals]]

### From interaction-picture Dyson simulation (arXiv:1805.00675)
- [[Interaction-Picture Norm Reduction]]
- [[Oracle Conjugation for Time-Dependent Encodings]]
- [[Average-Variation Discretization Budgeting]]

### From A Theory of Trotter Error (arXiv:1912.08854)
- [[Trotter Commutator-Scaling Bound]]
- [[Product-Formula Error Representation Switching]]
- [[Finite Nested-Commutator Expansion]]
- [[Order-Condition Cancellation in Product Formulas]]
- [[Interaction-Picture Split for Product Formulas]]
- [[Local-Observable-First Trotterization]]

### From recursive structured Trotter simulation (arXiv:2211.09133)
- [[Recursive Interval Decomposition for Power-Law Hamiltonians]]
- [[Hierarchical Low-Rank Hamiltonian Decomposition]]

### From symmetry-protected digital simulation (PRX Quantum 2, 010323 / arXiv:2006.16248)
- [[Symmetry-Rotated Error Averaging]]
- [[Deterministic Symmetry Cycle for Leading-Error Cancellation]]
- [[Randomized Symmetry Protection Between Trotter Steps]]
- [[Zeno-Style Projection via Symmetry Kicks]]

### From qDRIFT randomized simulation (arXiv:1811.08017)
- [[Term-Weighted Random Hamiltonian Sampling (qDRIFT)]]
- [[Fixed-Angle Randomized Product Formula]]
- [[Diamond-Norm Channel Averaging for Random Compilers]]
- [[Error-Budget Allocation Across Controlled-Time Schedules]]

### From improved QLSA precision scaling (arXiv:1511.02306)
- [[Fourier Integral LCU for Matrix Inversion]]
- [[Chebyshev-Regularized Inverse Approximation]]
- [[Quantum-Walk Realization of Chebyshev Matrix Polynomials]]
- [[Variable-Time Amplification for Condition-Number Reduction]]

### From linear ODE simulation via QLSA encoding (arXiv:1701.03684)
- [[Taylor-Series-to-Linear-System Encoding]]
- [[History-State Padding for Final-Time Readout]]
- [[Step-Size-Taylor-Order Balancing for ODE Simulation]]
- [[Postselection-Cost Parameterization via g-Factor]]

### From structured PDE simulation (Quantum q-2021-11-10-574)
- [[QFT Diagonalization for Circulant Laplacians]]
- [[QSFT Factorization into Diagonal + QFT + Diagonal]]
- [[QCT Implementation via QFT Decomposition]]
- [[Method-of-Images Boundary Embedding for Quantum PDE Solvers]]

### From black-box walk-era simulation (arXiv:0910.4157)
- [[Quantum-Walk Isometry Encoding for Black-Box Hamiltonians]]
- [[Lazy-Walk Phase-Correction for Simulation Accuracy]]
- [[Amplitude-Amplified State Preparation inside Walk Operators]]
- [[Magnitude-Banded Hamiltonian Decomposition]]
- [[Block-Hamiltonian Embedding for Unitary Synthesis]]

### From randomized product formulas (Quantum q-2019-09-02-182)
- [[Permutation Averaging for Product-Formula Error Cancellation]]
- [[Mixing-Lemma Upgrade from Average-Channel Error to Diamond Norm]]
- [[Nondegenerate-vs-Degenerate Taylor-Term Bookkeeping]]
- [[Monte-Carlo Permutation Error Estimation for Product Formulas]]

### From time-dependent L1-scaling simulation (Quantum q-2020-04-20-254)
- [[Continuous qDRIFT Time-Sampling by Instantaneous Norm]]
- [[Integrated-Norm Equal-Segment Partitioning]]
- [[Time-Reparameterization for Norm-Flattened Dyson Simulation]]
- [[Mixed-Unitary Channel Approximation for Time-Dependent Evolution]]

### From non-sparse simulation limitations (arXiv:0908.4398)
- [[Hardness Embedding via Circulant Hamiltonian Eigenphases]]
- [[Norm-Gap Obstruction between H and abs(H) in Oracle Simulation]]
- [[Diagonal Gauge Phase Removal on Tree-Structured Hamiltonians]]
- [[Average-Case Lower-Bound Transfer via Randomization and Concentration]]

### From coupled-oscillator quantum simulation speedup paper
- [[Square-Root Dynamics via Block Hamiltonian Lifting]]
- [[Incidence-Matrix Encoding of Coupled-Oscillator Forces]]
- [[Energy-Observable Extraction from Amplitude-Encoded Dynamics]]
- [[Inequality-Testing State Preparation for Sparse Weighted Graphs]]

### From sublinear-T fermionic block-encoding paper (arXiv:2510.08644)
- [[SELECT-SWAP Grouped Data Lookup for Block-Encoding]]
- [[SWAP-UP Routing for Fermionic Sign Operations]]
- [[Comparator-Based Direct Amplitude Sampling from Fixed-Point Coefficients]]
- [[Indirect Diffusion over Occupied-Index Sets]]
- [[Occupation-Detection Assisted Subspace Block-Encoding]]

### From non-normal eigenvalue transformation framework (arXiv:2401.06240)
- [[Generating-Function-to-QLSA Reduction for Polynomial Basis Histories]]
- [[Lower-Shift Ancilla Encoding for Polynomial Degree Management]]
- [[Chebyshev-State Phase Estimation for Real-Spectrum Non-Normal Inputs]]
- [[Faber-Polynomial Region Mapping for Complex-Spectrum Eigenvalue Transforms]]
- [[Fourier-Convolution Coefficient-State Preparation in Polylog Time]]

## Card template (for future additions)

- What it does
- The trick
- When to reach for it
- Complexity
- Caveat
- Source papers
