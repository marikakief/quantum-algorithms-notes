...
Each $T_k$ is block-diagonal with $2 \times 2$ blocks (sites $\{2i-1,2i\}$ for $T_1$, sites $\{2i,2i+1\}$ for $T_2$ in one-indexed notation). For a uniform hopping chain, all blocks are identical — each is $t_0 \begin{pmatrix} 0 & 1 \\ 1 & 0 \end{pmatrix}$.

To diagonalise in the identical-block case: map state $|n\rangle \to |(n+1) \text{ div } 2, \; n \bmod 2\rangle$ with the same indexing convention. The first register labels the block, the second is position within the block. Since $T_k$ acts identically on every block, apply a single $2 \times 2$ rotation on the second register — all blocks are diagonalised simultaneously.

For nonidentical but disjoint blocks, the terms still commute and can be applied in parallel, but the block label must control different angles/rotations.
...
- Higher-dimensional lattices: colour the interaction graph so each colour class has disjoint edges. Graph colouring gives a constant number of groups only when the degree is bounded independently of system size.

## Complexity
$O((\ln m)^2)$ operations per sublattice term in the relabeling/arithmetic model used by Abrams-Lloyd. In an explicit circuit layout, routing and basis-change costs can differ. The number of sublattice groups is $O(1)$ for bounded-degree local interactions.

## Caveat
Works most cleanly for translationally invariant Hamiltonians where all blocks are identical. Non-uniform couplings preserve disjoint block structure but require per-block phase advances or controlled rotations. This is a product-formula grouping trick: terms within a color commute, while terms in different colors generally do not.
