...
> **Source:** arXiv:2510.07380 (Quantum FMM, Berry, Wan, Baczewski, Tikku, Babbush); builds on Low et al. [[Product Formulas]] error bounds

## What it does
Tightens Trotter error bounds by computing commutator norms **restricted to the physical $\eta$-particle subspace** rather than the full Hilbert space. This is what makes the high-order [[Product Formulas]] approach competitive in the FMM paper — the step count depends on the restricted norm, not the full-space norm.

## The trick
For a Hamiltonian $H = T + V$ (kinetic + Coulomb potential), the Trotter error for an order-$p$ [[Product Formulas]] scales with $\|[T, V]\|_\text{phys}^{1/p}$, the commutator norm on the $\eta$-particle antisymmetric subspace.
...
- Justifying high-order [[Product Formulas]]s (order $p \gg 2$) for near-linear time scaling
