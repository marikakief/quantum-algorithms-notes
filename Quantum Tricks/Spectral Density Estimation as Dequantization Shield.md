...
## What it does
Provides a structural argument for why certain quantum algorithms resist broad, structure-blind dequantization: if the core computational primitive is spectral density estimation (rather than matrix inversion or SVD), generic classical simulation is DQC1-hard.
...
The DQC1-hardness of LLSD means that any classical algorithm solving the general sparse-matrix version would simulate DQC1 circuits, which is believed impossible in polynomial time. Therefore, quantum algorithms whose core subroutine is LLSD (rather than matrix inversion) avoid the broad low-rank dequantization route.

This applies most directly to general sparse PSD matrices, numerical rank estimation, spectral entropy estimation, and algorithms that fundamentally count eigenvalues. For [[Quantum Algorithms for Topological and Geometric Analysis of Data (Lloyd-Garnerone-Zanardi 2016) — Paper Notes|quantum TDA]], the extra question is whether the relevant clique-complex Laplacians retain the same hardness.
